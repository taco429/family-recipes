...
test.describe('Family Recipes App', () => {
  ...
  test('navigation menu works on desktop', async ({ page, isMobile }) => {
    ...
    // Navigate to Browse Recipes (on desktop it's an icon button with aria-label)
  ...
  test('app uses brutalist theme colors', async ({ page }) => {
    await page.goto('/');

    // Check that the AppBar has the correct black background color
    const appBar = page.locator('header');
    await expect(appBar).toHaveCSS('background-color', 'rgb(0, 0, 0)');
  });
});
